...
export default function Dashboard() {
  ...
  const getStatusCounts = () => {
    const published = blogs.filter((a) => a.status === "published").length;
    const draft = blogs.filter((a) => a.status === "draft").length;
    const archived = blogs.filter((a) => a.status === "archived").length;
  ...
  const loadAllTags = useCallback(async () => {
    try {
      const data = await blogAPI.getMy();
      const uniqueTags = [
  ...
  const loadArticles = useCallback(async () => {
    console.log("[DASHBOARD] Loading articles with:", {
      search,
      statusFilter,
      selectedTags,
    });
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <p>
              {search || statusFilter || selectedTags.length > 0
            ...
            <div className="space-y-4">
              {blogs.map((blog: Blog) => (
                <div
                  ...
                  onClick={() => router.push(`/editor?id=${blog.blog_id}`)}
                >
